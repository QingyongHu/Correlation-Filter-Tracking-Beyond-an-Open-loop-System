...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	otherwise
		...
		%call tracker function with all the relevant parameters        
        [positions, output_positions, time] = tracker(video_path, img_files, pos, target_sz, ...
            padding, kernel, lambda, output_sigma_factor, interp_factor, ...
            cell_size, features, show_visualization,m_T,n_theta,num_init,response_threshold,psr_threshold);

        rects = output_positions;
        [aveErrCoverage, aveErrCenter,errCoverage, errCenter_initial] = calcSeqErrRobust(rects, ground_truth);
        overlap = sum(errCoverage > thresholdOverlap)/length(errCoverage);
        precision = sum(errCenter_initial <= thresholdPrecision)/length(errCenter_initial);
        fn = numel(img_files);
        fps = fn / time;
