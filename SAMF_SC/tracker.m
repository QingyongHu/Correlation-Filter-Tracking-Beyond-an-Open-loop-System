...
% 	window_sz = 2 .^ nextpow2(window_sz);
    ...
    response = zeros(size(cos_window,1),size(cos_window,2),size(search_size,2));%Peak_res
            ...
            if frame >= num_init
                ...
                if (Peak_res(frame) < response_threshold && (temp_init > 0.35)) || (PSR_res  < PSR_threshold)
                    ...
                    for index_candidate = 1:length(candidate_position)
                        ...
                        tmp_sz = floor((target_sz * (1 + padding))*search_size(1));
                        param0 = [pos(2), pos(1), tmp_sz(2)/window_sz(2), 0,...
                            tmp_sz(1)/window_sz(2)/(window_sz(1)/window_sz(2)),0];
                        param0 = affparam2mat(param0);
                        patch = uint8(warpimg(double(im), param0, window_sz));
